...
| 수단 | 지금 상태 |
|---|---|
| 카카오페이 | **실제 연동.** `KAKAOPAY_SECRET_KEY` 가 있으면 켜집니다 |
| 신용·체크카드 | 카드사 선택까지 만들어 뒀고, 승인은 **모의**입니다 |
| 계좌이체 | 은행 선택까지 만들어 뒀고, 승인은 **모의**입니다 |
...
- **잔액이 브라우저에 있습니다.** 스냅샷에 담겨 저장되므로, 마음먹으면 손댈 수
  있습니다. 실제 서비스로 쓰려면 서버에 잔액 원장을 두고 차감도 서버에서 해야
  합니다.
- 준비한 결제를 서버 메모리에 들고 있습니다. 서버를 여러 개 띄우면 DB 로
  옮겨야 합니다.
- 환불·결제 취소 API 는 붙이지 않았습니다.
