...
def _시간대응답(줄) -> str:
    import json as _j
    return _j.dumps({"목적": "다이어트", "루틴명": "하루 나눠 하기", "한마디": "시간대마다 달라요",
                     "왜": ["시간대마다 달라요"], "주의": [], "동작": 줄}, ensure_ascii=False)


def test_시간대마다_따로_짓는다(monkeypatch):
    """예현(2026-09-21): "AI추천에서 시간대별로 루틴 정해주는거 다 똑같은 루틴을 각 시간대별로 하는거야? 시간대 별로
    피로도나 온도 같은게 다르니까 시간대별로 짜줬음 좋겠어". 줄마다 '시간대' 를 받고, 아침 → 밤 · 준비 → 본 → 정리 순으로 세운다."""
    _fake_sdk(monkeypatch, _시간대응답([
        {"시간대": "저녁", "코드": _코드("본운동", 1), "단계": "본운동", "수행량": "12회 × 3세트"},
        {"시간대": "아침", "코드": _코드("본운동"), "단계": "본운동", "수행량": "10회"},
        {"시간대": "아침", "코드": _코드("준비운동"), "단계": "준비운동", "수행량": "1분"},
        {"시간대": "저녁", "종목": "running", "단계": "본운동", "수행량": "20분"},
        {"시간대": "밤", "코드": _코드("정리운동"), "단계": "정리운동", "수행량": "2분"},
        {"시간대": "아침", "코드": _코드("정리운동"), "단계": "정리운동", "수행량": "1분"},   # 같은 동작을 다른 시간대에 — 둘 다 남는다
        {"시간대": "밤", "코드": _코드("정리운동"), "단계": "정리운동", "수행량": "또"},     # 같은 시간대에 두 번 — 뒤는 버린다
        {"코드": _코드("정리운동", 1), "단계": "정리운동", "수행량": "1분"},               # 시간대를 빠뜨림 — 바로 앞 줄(밤)로
    ]))
    r = air.compose(사용자, "성인", 종목ids=["running"])["루틴"]
    assert r["시간대별"] is True
    assert [(s["시간대"], s["단계"]) for s in r["steps"]] == [
        ("아침", "준비운동"), ("아침", "본운동"), ("아침", "정리운동"),
        ("저녁", "본운동"), ("저녁", "본운동"), ("밤", "정리운동"), ("밤", "정리운동")]
    assert [s["시간대"] for s in r["steps"] if s.get("코드") == _코드("정리운동")] == ["아침", "밤"]
    assert [s["수행량"] for s in r["steps"] if s["시간대"] == "저녁"] == ["12회 × 3세트", "20분"]   # 한 시간대 안에서는 받은 차례대로


def test_시간대를_나누지_않은_응답은_예전처럼_한_벌(monkeypatch):
    """시간대가 없거나 하나뿐이면 시간대를 지우고 한 벌로 둔다 — 화면은 예전처럼 시간대마다 그 한 벌을 보여 준다."""
    _fake_sdk(monkeypatch, _지은응답())                                          # 줄에 시간대가 없다
    r = air.compose(사용자, "성인", 종목ids=["running"])["루틴"]
    assert r["시간대별"] is False and all(s["시간대"] is None for s in r["steps"])
    _fake_sdk(monkeypatch, _시간대응답([
        {"시간대": "저녁", "코드": _코드("준비운동"), "단계": "준비운동"},
        {"시간대": "저녁", "코드": _코드("본운동"), "단계": "본운동"},
        {"시간대": "새벽", "코드": _코드("정리운동"), "단계": "정리운동"}]))          # 모르는 시간대는 앞 줄을 따른다 → 저녁 하나뿐
    r = air.compose(사용자, "성인")["루틴"]
    assert r["시간대별"] is False and all(s["시간대"] is None for s in r["steps"])


def test_지시문이_시간대마다_따로_짓게_한다():
    s = air.COMPOSE_SYSTEM
    assert "**오늘 하루의 운동을 시간대(아침 · 낮 · 저녁 · 밤)마다 따로**" in s
    assert "**하루를 네 시간대로 나눠 시간대마다 따로 짭니다.** 줄마다 '시간대' 를 적습니다." in s
    for 때, 뜻 in (("아침", "몸이 굳어 있고 기온이 낮습니다"), ("낮", "기온이 가장 높습니다"),
                  ("저녁", "본운동의 중심"), ("밤", "잠들기 전입니다")):
        assert f"{때}" in s and 뜻 in s, 때
    assert "시간대마다 용량을 다 채우면 하루 네 배가 됩니다" in s                   # 네 번 되풀이하지 않는다
    assert "'시간대기온' 이 있으면" in s and "퇴근 · 하교 뒤의 피로" in s             # 온도 · 피로도
    assert '{"시간대": "아침", "코드": "V…"' in s
    assert air.COMPOSE_MAX_STEPS >= 12 and air.DAYPARTS == ("아침", "낮", "저녁", "밤")


def test_조정할_때도_시간대_나눔째로_넘긴다():
    from backend.main import _previous_routine
    d = _previous_routine({"루틴명": "x", "steps": [
        {"동작": "스쿼트", "단계": "본운동", "수행량": "10회", "시간대": "저녁"},
        {"동작": "목 스트레칭", "단계": "준비운동", "수행량": "1분", "시간대": "새벽"}]})
    assert d["동작"][0]["시간대"] == "저녁" and "시간대" not in d["동작"][1]           # 모르는 시간대는 버린다
