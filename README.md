...
`목록 → 루틴 추천` 은 기본적으로 **측정값·스타일 테스트·고른 종목·목표 격차**로 점수를 매겨
250개 고정 루틴 중에서 고릅니다. 여기에 키를 넣으면 **Claude 가 같은 후보 안에서**
순서와 설명만 다듬습니다. 루틴을 새로 만들지는 않습니다.
...
추천 카드 아래가 "AI 가 순서와 설명을 다듬었어요" 로 바뀝니다.

**키가 없어도 추천은 그대로 동작합니다.** 호출이 실패하거나 모델이 후보에 없는 번호를
내면 그 응답을 버리고 점수 결과를 씁니다 — 화면이 비는 경우는 없습니다.

### 추천 두 가지 방식과 이용권

| | 무료 추천 | AI 추천 |
|---|---|---|
| 고르는 방법 | 측정값·스타일 테스트·고른 종목·목표 격차로 점수 | 같은 후보를 Claude 가 다시 배열하고 설명을 다시 씀 |
| 값 | 없음 | **1회 100원** (미리 충전) |
| 키 없을 때 | 그대로 동작 | 잠김 |

이용권은 프로필의 **AI 추천 이용권** 카드에서 미리 충전해 둡니다. 잔액·남은 횟수·최근 내역이 거기 있습니다.

> ⚠️ **지금 결제는 시연용 모의 승인입니다.** 카드 정보를 받지 않고 실제로 돈이 빠져나가지 않습니다.
> 실제 결제를 붙이려면 사업자 등록과 PG 계약이 필요하고, 카드 번호는 앱이 직접 받지 않고 PG 결제창에 맡겨야 합니다.
> 잔액은 지금 사용자 기록(스냅샷) 안에 있어 임의로 고칠 수 있습니다 — 실제 과금을 하려면 서버에서 잔액을 관리해야 합니다.
