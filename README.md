...
사진 2, 한 곳(IP)에서 합계 60 — 같은 와이파이의 여러 사람이 막히지 않게)로 제한합니다. `ADMIN_USERS`(`kakao:회원번호` 처럼 `/auth/me` 의 `계정` 값, 소셜 계정은 이메일도 됨 — 비밀번호 계정은 이메일을 확인한 적이 없어 `password:이메일` 로만)에 적힌
