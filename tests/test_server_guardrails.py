...
def test_AI_경로의_제한은_AI_가_스스로_포기하는_시간보다_길다():
    ...
    가장_긴 = max(air.TIMEOUT_SEC * 2, air.COMPOSE_TIMEOUT_SEC, air.PERIOD_TIMEOUT_SEC * 1.5)
    assert air.longest_wait_sec() == 가장_긴 * (air.SDK_RETRIES + 1)
    ...
    assert src.count("max_retries=SDK_RETRIES)") == 4 and "max_retries=1" not in src


@pytest.fixture
def 이용권_있는_사람(monkeypatch):
    """가입시키고 이용권 300원을 넣는다. AI 는 '쓸 수 있다' 로만 둔다 — 실제로 부르지 않는다."""
    ...
    billing.charge(uid, 300, 300, f"order-{uuid.uuid4().hex[:10]}")
...
def test_제한_안에_지어지면_루틴을_주고_값을_받는다(이용권_있는_사람, monkeypatch):
    ...
    assert r.status_code == 200 and r.json()["출처"] == "ai" and r.json()["잔액"] == 200
    assert billing.balance(uid) == 200
...
def test_끊긴_뒤에_지어진_루틴은_값을_받지_않는다(이용권_있는_사람, monkeypatch):
    ...
    assert billing.balance(uid) == 300                      # 화면은 실패를 받았다 — 한 푼도 받지 않는다


def test_끊긴_뒤에_읽힌_시간표_사진도_값을_받지_않는다(이용권_있는_사람, monkeypatch):
    from backend import billing
    c, uid = 이용권_있는_사람
    ...
    assert billing.balance(uid) == 300
    # 제때 읽히면 받는다
    ...
    r = c.post("/schedule/photo", json={"사진": "data:image/png;base64,AAAA"})
    assert r.status_code == 200 and r.json()["잔액"] == 200
