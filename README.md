...
```
⓪ 시작   닉네임만 입력 → 프로필 (같은 기기에서 여러 명 사용 가능)
            ↓
...
측정값은 개인 건강정보입니다. 시제품 단계에서 비밀번호를 저장하면 사고가 나기 쉽고,
나면 되돌릴 수 없습니다. 그래서 **계정 서버를 두지 않았습니다.**

- 닉네임(또는 이메일)만 입력하면 시작합니다
- 프로필은 그 브라우저의 `localStorage` 에만 남고 서버로 가지 않습니다
- 같은 기기를 여러 명이 쓸 수 있습니다 (가족 단위 사용 대응)
- 이미 점검한 사람은 로그인 즉시 홈으로, 처음이면 첫 점검으로 갑니다

실제 배포에서 계정이 필요해지면 `frontend/index.html` 의 `login()` 자리를
OAuth 같은 걸로 갈아끼우면 됩니다. 화면 흐름은 그대로입니다.
