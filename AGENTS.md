...
- **PR 본문은 AI가 "무엇을 / 왜"까지 다 써도 됩니다.** 대신 올리는 사람이 읽고,
  사실과 다르거나 자기 판단이 아닌 부분은 고친 뒤에 리뷰를 요청합니다.
  리뷰어가 알아야 하는 건 코드 설명이 아니라 *왜 이렇게 했는지* 입니다.
  AI가 쓴 본문은 맨 아래에 한 줄로 표시해두세요.
- PR 하나에 파일이 10개를 넘으면 쪼개세요.
- **본인이 설명하지 못하는 코드와 PR은 올리지 않습니다.** 본문을 누가 썼든 같습니다.
  리뷰에서 "왜 이렇게 했냐" 는 질문에 답하는 건 AI가 아니라 올린 사람입니다.
