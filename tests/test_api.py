...
def _시작한_state(c, provider: str) -> str:
    """/auth/{provider}/start 가 하는 일 — DB 에 state 를 만들고 같은 값을 이 브라우저의 쿠키에 둔다."""
    from backend import auth
    s = auth.new_state(provider)
    c.cookies.set(auth.OAUTH_COOKIE, s)
    return s
...
def test_소셜_로그인이_실패하면_어디서_왜인지_알려_준다(monkeypatch, capsys):
    ...
    q = 실패("/auth/google/callback?error=access_denied&error_description=not+a+test+user&state=" + _시작한_state(c, "google"))
    ...
    q = 실패("/auth/google/callback?code=code-SECRET-456&state=" + _시작한_state(c, "google"))
    ...
    assert 실패("/auth/google/callback?code=code-SECRET-456&state=" + _시작한_state(c, "google"))["why"] == "profile"
    ...
    r = c.get("/auth/google/callback?code=abc&state=" + _시작한_state(c, "google"), follow_redirects=False)
    assert r.headers["location"] == "/?login=ok" and "quadriga_session" in r.headers.get("set-cookie", "")


def test_소셜_state는_시작한_브라우저에서만_통한다(monkeypatch):
    """로그인 CSRF — 공격자가 제 브라우저로 시작한 로그인의 콜백 주소를 남에게 열게 하면, 그 사람의 세션이
    공격자 계정으로 묶여 그 뒤로 적는 기록이 공격자에게 보인다. 그래서 state 는 DB 에 있는 것만으로는 모자라고
    **이 브라우저가 시작한 것**(쿠키가 같은 값)이어야 한다. 시작하면 쿠키가 생기고, 콜백이 끝나면 지워진다."""
    from urllib.parse import parse_qs, urlparse
    from backend import auth
    monkeypatch.setenv("GOOGLE_CLIENT_ID", "test-id")
    monkeypatch.setenv("GOOGLE_CLIENT_SECRET", "test-secret-XYZ")
    _가짜_제공자(monkeypatch, {"access_token": "tok"}, {"sub": "g-777", "email": "ok@ex.com", "name": "가"})

    공격자 = _fresh()
    r = 공격자.get("/auth/google/start", follow_redirects=False)
    state = parse_qs(urlparse(r.headers["location"]).query)["state"][0]
    assert 공격자.cookies.get(auth.OAUTH_COOKIE) == state                 # 시작한 브라우저에 같은 값이 남는다
    assert "HttpOnly" in r.headers["set-cookie"] and "Path=/auth/" in r.headers["set-cookie"]

    피해자 = _fresh()                                                      # 쿠키가 없는 다른 브라우저
    r = 피해자.get(f"/auth/google/callback?code=abc&state={state}", follow_redirects=False)
    assert parse_qs(urlparse(r.headers["location"]).query)["why"] == ["state"]
    assert "quadriga_session" not in r.headers.get("set-cookie", "")
    피해자.cookies.set(auth.OAUTH_COOKIE, "not-mine")                        # 제 것이 아닌 state 를 쿠키에 넣어도
    r = 피해자.get(f"/auth/google/callback?code=abc&state={state}", follow_redirects=False)
    assert parse_qs(urlparse(r.headers["location"]).query)["why"] == ["state"]

    # 시작한 브라우저는 그대로 된다(제 계정으로 제가 들어가는 것이라 막을 까닭이 없다) — 끝나면 state 쿠키가 지워진다
    r = 공격자.get(f"/auth/google/callback?code=abc&state={state}", follow_redirects=False)
    assert r.headers["location"] == "/?login=ok" and "quadriga_session" in r.headers.get("set-cookie", "")
    assert not 공격자.cookies.get(auth.OAUTH_COOKIE)
    # 같은 state 로 한 번 더는 안 된다 — 쓰고 버렸다
    공격자.cookies.set(auth.OAUTH_COOKIE, state)
    r = 공격자.get(f"/auth/google/callback?code=abc&state={state}", follow_redirects=False)
    assert parse_qs(urlparse(r.headers["location"]).query)["why"] == ["state"]

    # 10분이 지난 state 는 DB 에 남아 있어도 안 된다
    낡은 = _시작한_state(공격자, "google")
    assert auth.take_state(낡은, now=int(__import__("time").time()) + auth.OAUTH_STATE_SEC + 1) is None
...
@needs_data
def test_성장기_근지구력은_공식_등급_기준으로_또래_순위를_어림한다():
    """성장기의 근지구력 항목(윗몸말아올리기·반복점프)은 아직 분포 파일에 없다. 국민체력100 인증기준의 1·2·3등급 값이
    곧 또래 상위 30·50·70% 라서(고시 제6조) 그 세 점으로 백분위를 어림한다 — 체력나이(발달 수준)에는 넣지 않는다."""
    보냄 = {"age_gbn": "성장기", "sex": "M", "age": 15, "flexibility": 9,
           "strength": 200, "height_cm": 168, "weight_kg": 58}
    안잼 = client.post("/fitness-age", json=보냄).json()
    assert "근지구력" not in 안잼["또래비교"]
    def 근지구력(회, **더):
        return client.post("/fitness-age", json={**보냄, **더, "muscle_endurance": 회}).json()
    b = 근지구력(50)                                          # 남 15세 반복점프 1등급 기준 = 50회
    assert b["체력나이"] == 안잼["체력나이"] and "근지구력" not in b["항목별"]
    r = b["또래비교"]["근지구력"]
    assert r["항목"] == "반복점프" and r["등급"] == 1 and r["백분위"] == 70 and r["또래중앙값"] == 44
    assert r["어림"] is True and r["표본수"] == 0 and "인증기준" in r["근거"]
    assert 근지구력(44)["또래비교"]["근지구력"]["백분위"] == 50       # 2등급 기준 = 또래 상위 50%
    assert 근지구력(38)["또래비교"]["근지구력"]["백분위"] == 30       # 3등급 기준 = 또래 상위 70%
    낮음 = 근지구력(20)["또래비교"]["근지구력"]
    assert 낮음["등급"] is None and 낮음["백분위"] == 5                # 끝은 5~95 에서 자른다
    assert 근지구력(90)["또래비교"]["근지구력"]["백분위"] == 95
    유소년 = 근지구력(27, age=12, sex="F")["또래비교"]["근지구력"]    # 만 12세까지는 윗몸말아올리기
    assert 유소년["항목"] == "윗몸말아올리기" and 유소년["등급"] == 2 and 유소년["백분위"] == 50
    # 성인의 근지구력은 strength 다 — 이 값은 쓰지 않는다
    성인 = client.post("/fitness-age", json={**ADULT, "age": 30, "muscle_endurance": 50}).json()
    assert 성인["또래비교"]["근지구력"]["항목"] == "교차윗몸일으키기"


def test_성장기_근지구력_기준표는_공식_값이다():
    """https://nfa.kspo.or.kr 인증기준 표(= 문체부고시 제2025-27호 별표2·5)에서 옮긴 값. (1등급, 2등급, 3등급)"""
    from backend import fitness_age as fa
    점프, 말아 = fa.GROWTH_ENDURANCE["반복점프"], fa.GROWTH_ENDURANCE["윗몸말아올리기"]
    assert 점프["M"][13] == (44, 39, 33) and 점프["M"][18] == (52, 46, 40)
    assert 점프["F"][13] == (31, 27, 22) and 점프["F"][18] == (36, 29, 23)
    assert 말아["M"] == {11: (36, 26, 19), 12: (35, 27, 19)} and 말아["F"] == {11: (36, 26, 18), 12: (35, 27, 19)}
    for 표 in (점프, 말아):
        for 성별 in ("M", "F"):
            assert all(c1 > c2 > c3 > 0 for c1, c2, c3 in 표[성별].values())
    assert sorted(점프["M"]) == sorted(점프["F"]) == list(range(13, 19))
...
def test_방침이_실제로_하는_일을_적는다():
    """2차 점검 A-2 — 위치 · 건강 상태(민감정보) · Anthropic 국외 이전 · 카카오페이 · 커뮤니티 사진이 방침에 있어야 한다.
    있는데 안 적은 것이 제일 나쁘다."""
    t = _fresh().get("/privacy").text
    for 항목 in ("위치정보 (선택)", "민감정보", "Anthropic", "국외 이전", "카카오페이", "커뮤니티 (선택)", "메타데이터를 지운 뒤",
               "만 14세 이상임을 확인", "체력인증센터 측정건수 정보", "5년"):
        assert 항목 in t, 항목
    assert "위치정보, 연락처·주소록" not in t                     # 위치를 안 받는다고 적어 두던 줄은 사라졌다


def test_약관에_유료_기능과_환불_규정이_있다():
    """값표(#249) · 구독(#252)이 들어갔는데 약관은 '무료 서비스' 로만 적혀 있었다 — 유료 조항과 환불 · 자동 갱신 없음 · 시연 기간을 적는다."""
    t = _fresh().get("/terms").text
    for 항목 in ("제6조의2", "이용권", "한 달 구독", "9,900원", "자동으로 갱신되지 않으며", "환불", "7일 안에", "시연 기간", "테스트 가맹점"):
        assert 항목 in t, 항목
    assert "무료로 제공되는 서비스이므로" not in t


def test_방침에_DB_위탁과_출퇴근_경로가_있다():
    """2026-09-25 점검 — DB 는 9/24 부터 Neon(미국)인데 방침에는 Render 가 'DB 운영' 으로 남아 있었고, 로그인하면 서버에
    남는 출퇴근 출발지 · 목적지와 카카오 장소 · 도보 경로 조회가 빠져 있었다. AI 이용 기록의 IP 보관도 실제대로 적는다."""
    t = _fresh().get("/privacy").text
    for 항목 in ("Neon, Inc.", "데이터베이스 운영", "출퇴근 경로 (선택)", "도보 경로", "경로 고도", "다음 날 지웁니다", "회원번호를 지워"):
        assert 항목 in t, 항목
    assert "서버 호스팅, 데이터베이스 운영" not in t


def test_약관에_커뮤니티_게시물_규칙과_데이터_출처가_있다():
    """커뮤니티가 있는데 게시물 규칙이 없었고, OSM(ODbL) · Open-Meteo(CC BY 4.0) 는 출처 표기가 조건이다."""
    t = _fresh().get("/terms").text
    for 항목 in ("제5조의2", "커뮤니티 게시물", "신고", "OpenStreetMap", "ODbL", "Open-Meteo", "CC BY 4.0"):
        assert 항목 in t, 항목
