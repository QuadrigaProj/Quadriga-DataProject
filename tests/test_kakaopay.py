...
def 준비(c, monkeypatch, 이용권=3450, 기록=None):
...
def test_선결제_할인표는_서버가_내려준다():
    ...
    assert 표 == {500: (500, 0), 1100: (1000, 9), 3450: (3000, 13), 6000: (5000, 17), 12500: (10000, 20)}
    보너스 = {p["결제"]: p["보너스"] for p in packs}
    assert 보너스 == {500: 0, 1000: 10, 3000: 15, 5000: 20, 10000: 25}


def test_키가_없으면_준비도_안_한다():
    assert _login().post("/pay/kakao/ready", json={"amount": 3450}).status_code == 503
...
def test_로그인하지_않으면_결제할_수_없다(키):
    ...
    c = TestClient(app)
    assert c.post("/pay/kakao/ready", json={"amount": 3450}).status_code == 401
...
def test_고를_수_없는_금액은_거절한다(키):
    ...
    assert a.post("/pay/kakao/ready", json={"amount": -3450}).status_code == 400
    # 결제 금액을 이용권 자리에 넣어도 안 통한다 (3,000원짜리 이용권 팩은 없다)
...
def test_준비하면_결제창_주소를_주고_주문을_남긴다(키, monkeypatch):
    ...
    assert o["status"] == "ready" and o["credit"] == 3450 and o["amount"] == 3000


def test_1회만_결제는_할인이_없다(키, monkeypatch):
    기록 = []
    d = 준비(_login(), monkeypatch, 500, 기록)
    assert d["결제"] == 500 and d["이용권"] == 500
    assert 기록[0]["body"]["total_amount"] == 500
...
def test_준비_응답이_모자라면_502(키, monkeypatch, 응답):
    ...
    assert _login().post("/pay/kakao/ready", json={"amount": 1100}).status_code == 502
...
def 승인(c, order, 금액=3000, monkeypatch=None):
...
def test_승인하면_서버_잔액이_오른다(키, monkeypatch):
    ...
    assert c["잔액"] == 3450
    줄 = c["내역"][0]
    assert (줄["종류"], 줄["금액"], 줄["부호"], 줄["결제"], 줄["메모"])         == ("충전", 3450, 1, 3000, "")
    assert a.get(f"/pay/result/{order}").json()["amount"] == 3450
...
def test_같은_주문은_두_번_충전되지_않는다(키, monkeypatch):
    ...
    assert a.get("/credit").json()["잔액"] == 3450

    # 원장에 직접 한 번 더 넣으려 해도 막힌다
    billing.charge(1, 3450, 3000, order)
    assert a.get("/credit").json()["잔액"] == 3450


@pytest.mark.parametrize("승인액", [100, 1000, 2999, 3001, 3450, 999999])
...
@pytest.mark.parametrize("응답", [
    {}, {"amount": {}}, {"amount": {"total": 0}}, {"amount": {"total": "3450"}},
...
def test_취소하면_주문이_닫힌다(키, monkeypatch):
    ...
    r2 = 승인(a, order, 3000, monkeypatch)
...
def test_잔액은_원장의_합이다(키, monkeypatch):
    ...
    order = 준비(a, monkeypatch)["order"]
    승인(a, order, 3000, monkeypatch)
    assert billing.spend(1, 100, "AI 추천") == 3350
    assert a.get("/credit").json()["잔액"] == 3350
...
def test_환불하면_돈도_이용권도_되돌아간다(키, monkeypatch):
    ...
    assert r == {"ok": True, "환불": 3000, "잔액": 0}
    assert 기록[0]["url"].endswith("/online/v1/payment/cancel")
    assert 기록[0]["body"]["cancel_amount"] == 3000
    assert billing.get_order(order)["status"] == "refunded"
    내역 = a.get("/credit").json()["내역"]
    assert 내역[0]["종류"] == "환불" and 내역[0]["금액"] == 3450
...
def test_그_결제_전에_쓴_것은_따지지_않는다(키, monkeypatch):
    ...
    order = 준비(a, monkeypatch)["order"]
    승인(a, order, 3000, monkeypatch)
    assert a.get("/credit").json()["잔액"] == 3950
...
def test_세_주소를_모두_돌려준다(키, monkeypatch):
    ...
    d = _login().post("/pay/kakao/ready", json={"amount": 3450}).json()
...
def test_카카오_에러코드를_할_일로_바꿔_알려준다(키, monkeypatch, 코드, 조각):
    ...
    r = _login().post("/pay/kakao/ready", json={"amount": 500})
