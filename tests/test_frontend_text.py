...
추가된_문구 = [
    "맞춤 운동을 진행하다가, 원할 때 다시 측정해서 달라진 체력을 확인해보세요.",   # A2
    "집중 개선 영역: ${",                                                   # A3 동적
    "앞으로의 루이 이 항목 중심으로 구성됩니다.",                            # A3
...
def test_3개월_뒤_재점검_문구가_사라졌다():
    """3개월 뒤 재점검을 전제한 문구는 전부 없다."""
    html = _index()
    for gone in ("재점검까지 D-", "재점검할 때가 됐어요", "이번 3개월",
                 "3개월마다 다시 잴 수 있어요", "지난 3개월, 이렇게 바뀌었어요",
                 "3개월 뒤 다시 재보면",
                 "3개월간 맞춤 운동을 진행한 뒤, 다시 측정해 달라진 체력을 확인해보세요.",
                 "3개월 프로그램이 이 항목 중심으로 구성됩니다."):
        assert gone not in html, gone
